...
public:
	...
	Region(int boundary_size, int endpoint2_num, int id){
		region_id = id;
		endpoint1 = 0;
   ...
   // std::cout << " returning node " << boundary_nodes + internal_nodes << std::endl;
	...
	bool isValid(){
		...
		for (ListGraph::OutArcIt arcIt(g, g.nodeFromId(endpoint2)); arcIt != INVALID; ++arcIt) {

		    ListGraph::Arc arc(arcIt);
		    ListGraph::Node oppositeNode( g.oppositeNode(g.nodeFromId(endpoint2), arc) );
			//cout << "Found neighbor to " << g.id(endpoint2) << ": " << g.id(oppositeNode) << endl;
			dominated[g.id(oppositeNode)] = true;
		}
	...
	std::pair<int,int> getOverUnder(){
		// First endpoint is always 0
		int over = endpoint2-1;
		int under = C.size()-endpoint2-1;
		if(under > over) std::swap(over, under);
		std::pair<int,int> p(over, under);
		return p;
	}
	
	void getSignature(std::vector<int> &signature){
		signature.clear();
		std::pair<int, int> p = getOverUnder();
